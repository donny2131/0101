...
.setDescription(`اي شسمه اكتب اسم الرول`)
...
let embed = new Discord.RichEmbed().setImage(photo).setTitle(`متأكد انك تبي ترسل ذا ؟  \`[ي,لا]\`**`).setColor('#36393e')
message.channel.send(embed).catch(e =>{return message.channel.send('رابط الصورة غلط.')});
let filter = m => m.author.id == message.author.id
